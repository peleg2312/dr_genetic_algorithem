  ...
  void runGeneticAlgorithm() {
    ...
    for (int i = 0; i < generationCount; i++) {
      ...
      for (int i = 0; i < populationLength/5; i++) {
        crossover(population.individuals[rng.nextInt(populationLength~/5)], population.individuals[rng.nextInt(populationLength~/5)]);
      }
...
int countGenes(List<List<Gene>> genesList) {
  ...
  void crossover(Individual parent1, Individual parent2) {
    ...
    for (var gene in parent2.genes) {
      for(var g in gene){
        if(!_geneExists(child1.genes, g) && child1.doctors[g.doctor.Id]!.checkAvelability((g.time - g.doctor.startTime), g.day)){
          child1.genes[j].add(g);
          child1.doctors[g.doctor.Id]?.avelability[g.day][(g.time-g.doctor.startTime)*2] = true;
        }
        else if(!_geneExists(child1.genes, g) && !child1.doctors[g.doctor.Id]!.checkAvelability((g.time - g.doctor.startTime), g.day)){
          Doctor altDoctor = g.doctor;
          Gene newG = parent1.findPatient(g.patient.Id);
          if(child1.doctors[newG.doctor.Id]!.checkAvelability((newG.time - newG.doctor.startTime), newG.day) && !_geneExists(child1.genes, newG)){
    ...
    for (var gene in parent1.genes) {
      for(var g in gene){
        if(!_geneExists(child2.genes, g) && child2.doctors[g.doctor.Id]!.checkAvelability((g.time - g.doctor.startTime), g.day)){
          child2.genes[j].add(g);
          child2.doctors[g.doctor.Id]?.avelability[g.day][(g.time-g.doctor.startTime)*2] = true;
        }
        else if(!_geneExists(child2.genes, g) && !child2.doctors[g.doctor.Id]!.checkAvelability((g.time - g.doctor.startTime), g.day)){
          Doctor altDoctor = g.doctor;
          Gene newG = parent2.findPatient(g.patient.Id);
          if(child2.doctors[newG.doctor.Id]!.checkAvelability((newG.time - newG.doctor.startTime), newG.day) && !_geneExists(child2.genes, newG)){
